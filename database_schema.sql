-- XImoveis Database Schema (MySQL 8)
-- Execute com um usuario com permissao de criar banco e tabelas
...
CREATE TABLE property_certificates (
  ...
  -- Suporte a arquivos criptografados (GCM)
  encrypted BOOLEAN NOT NULL DEFAULT FALSE,
  enc_algo VARCHAR(20) NULL DEFAULT 'aes-256-gcm',
  enc_iv VARBINARY(16) NULL,
  enc_auth_tag VARBINARY(16) NULL,
...
-- Seed: uma agencia
INSERT INTO agencies (name, email) VALUES ('Agencia X', 'contato@agenciax.local');

-- Amostras de imoveis ACTIVE
-- Brasilia
INSERT INTO properties (
  ...
  NULL, NULL, 'Casa aconchegante no Lago Sul', 'Otima casa proxima ao lago.', 1250000.00, 4, 3, 'Brasilia', 'DF',
...
INSERT INTO properties (
  ...
  NULL, NULL, 'Apartamento na Vila Mariana', 'Apartamento com varanda e vista.', 780000.00, 2, 2, 'Sao Paulo', 'SP',
...
-- Historico para amostras
INSERT INTO property_history (property_id, event_date, event_type, price, source, notes)
SELECT id, NOW(), 'CREATED', price, 'seed', 'Imovel criado (seed)'
FROM properties WHERE city IN ('Brasilia', 'Sao Paulo', 'Rio de Janeiro');

INSERT INTO property_history (property_id, event_date, event_type, price, source, notes)
SELECT id, NOW(), 'STATUS_CHANGE', NULL, 'seed', 'Aprovado para ACTIVE'
FROM properties WHERE city IN ('Brasilia', 'Sao Paulo', 'Rio de Janeiro');

-- Metadados de certificados para amostras (dummy)
...
-- Garanta que o usuário #1 seja ADMIN por padrão (se existir)
UPDATE users SET role = 'ADMIN' WHERE id = 1;

-- Observação: se já possuir banco criado, aplique os ajustes equivalentes:
-- ALTER TABLE property_certificates
--   ADD COLUMN encrypted BOOLEAN NOT NULL DEFAULT FALSE AFTER sha256_hash,
--   ADD COLUMN enc_algo VARCHAR(20) NULL DEFAULT 'aes-256-gcm' AFTER encrypted,
--   ADD COLUMN enc_iv VARBINARY(16) NULL AFTER enc_algo,
--   ADD COLUMN enc_auth_tag VARBINARY(16) NULL AFTER enc_iv,
